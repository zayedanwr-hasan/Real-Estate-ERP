...
class ReceiptVoucherScreen:
    ...
    def _build_header_buttons(self):
        ...
        ttk.Label(header, text="LandLedger ERP - سند قبض نقدي", style="App.Receipt.Header.TLabel").pack(side="right", padx=30, pady=15)
    ...
    def _create_compact_field(
        ...
        **kwargs,
        ...
        grp = ttk.Frame(parent, style="App.Receipt.Content.TFrame")
        grp.grid(row=0, column=col, sticky="ew", padx=group_padx, pady=pady)
        grp.columnconfigure(0, weight=1 if expand else 0)

        if widget_type == "entry":
            field = ttk.Entry(grp, style="App.Receipt.Field.TEntry", justify="right", **kwargs)
        elif widget_type == "combo":
            field = ttk.Combobox(grp, style="App.Receipt.Field.TCombobox", justify="right", **kwargs)
        else:
        ...
        ttk.Label(
            grp,
            text=label_text,
            style="App.Receipt.FieldLabel.TLabel",
            width=label_width or self.field_label_width,
        ).grid(row=0, column=1, sticky="e")
        return field

    def _build_header_section(self, row):
        row_wrap = ttk.Frame(self.container, style="App.Receipt.Content.TFrame")
    ...
    def _build_amount_notes_section(self, row):
        ...
        self.ent_notes.bind("<KeyRelease>", lambda _e: self._sync_preview_line())
    ...
    def _create_date_field(self, parent, label_text, col=0, label_width=None, group_padx=(0, 0), pady=10):
        ...
        if date_class:
            field = date_class(grp, bootstyle="success", dateformat="%Y-%m-%d")
            field.entry.configure(justify="right", font=("Segoe UI", 11, "bold"))
        else:
            field = ttk.Entry(grp, style="App.Receipt.Field.TEntry", justify="right")
    ...
    def _build_lines_table(self, row):
        ...
        self.lines_tree = ttk.Treeview(table_area, columns=cols, show="headings", style="App.Receipt.Treeview", selectmode="browse")
    ...
    def _refresh_amount_words(self):
        ...
        if amount <= 0:
            self.amount_words_var.set("")
            return
        self.amount_words_var.set(self._amount_to_words_ar(amount))
    ...
    def load_beneficiaries(self):
        ...
        try:
            ...
            # Customers are intentionally loaded first to prioritize receipt workflow.
            cur.execute(
                """
    ...
    def _on_beneficiary_selected(self, *_):
        ...
        self.beneficiary_id_var.set(str(data.get("control_account") or ""))
        self._sync_preview_line()
    ...
    def _insert_ledger_row(self, cur, *, voucher_id, account_code, debit, credit, line_description, posting_date, vendor_id=None, customer_id=None):
        base_vals = (voucher_id, account_code, debit, credit, line_description, posting_date)

        if self.ledger_has_vendor_id and self.ledger_has_customer_id and self.ledger_has_property_id:
            cur.execute(
                """
                ...
                """,
                base_vals + (vendor_id, customer_id, None),
            )
        elif self.ledger_has_vendor_id and self.ledger_has_customer_id:
            cur.execute(
                """
                INSERT INTO finance.ledger (
                    voucher_id,
                ...
                """,
                base_vals + (vendor_id, customer_id),
            )
        elif self.ledger_has_vendor_id and self.ledger_has_property_id:
            cur.execute(
                """
                INSERT INTO finance.ledger (
                    voucher_id,
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """,
                base_vals + (vendor_id, None),
            )
        elif self.ledger_has_customer_id and self.ledger_has_property_id:
            cur.execute(
                """
                INSERT INTO finance.ledger (
                    voucher_id,
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """,
                base_vals + (customer_id, None),
            )
        elif self.ledger_has_vendor_id:
            cur.execute(
                """
                INSERT INTO finance.ledger (
                    voucher_id,
        ...
        elif self.ledger_has_customer_id:
            cur.execute(
                """
                INSERT INTO finance.ledger (
                    voucher_id,
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s)
                """,
                base_vals + (customer_id,),
            )
        elif self.ledger_has_property_id:
            cur.execute(
                """
                INSERT INTO finance.ledger (
                    voucher_id,
        ...
        try:
            ...
            if is_update:
                ...
                cur.execute(
                    ...
                    WHERE id = %s
                    """,
                    (ref_no, VOUCHER_TYPE_RECEIPT, date, notes, vid),
                )
                cur.execute("DELETE FROM finance.ledger WHERE voucher_id = %s", (vid,))
            else:
                cur.execute(
                    """
                    INSERT INTO finance.vouchers (reference_no, v_type, v_date, description)
                    VALUES (%s, %s, %s, %s)
                    RETURNING id
                    """,
                    (ref_no, VOUCHER_TYPE_RECEIPT, date, notes),
                )
                vid = cur.fetchone()[0]
            ...
            counterparty_code = str(ben.get("control_account") or "").strip()
            ...
            self._insert_ledger_row(
                ...
                debit=amount,
                credit=0,
                line_description=notes or "سند قبض",
                posting_date=date,
            )
            self._insert_ledger_row(
                cur,
    ...
    def _load_voucher_by_id(self, voucher_id):
        ...
        try:
            ...
            for row_data in ledger_rows:
                ...
                if debit > 0 and credit == 0 and not fund_set:
                    fund_display = self.account_code_to_display.get(acc_code) or acc_code
                    if self.fund_display_to_code.get("الصندوق الرئيسي") == acc_code:
                        self.combo_fund.set("الصندوق الرئيسي")
                    else:
                        self.combo_fund.set(fund_display)
                    fund_set = True
                    continue
    ...
    def _resolve_beneficiary_display(self, vendor_id, customer_id, account_code):
        for disp, data in self.beneficiary_display_to_data.items():
            ...
            if vendor_id is not None and data.get("type") == "vendor" and int(data.get("vendor_id", 0) or 0) == int(vendor_id):
                return disp
            if str(data.get("control_account") or "") == str(account_code or "") and not vendor_id and not customer_id:
                return disp
        return ""
